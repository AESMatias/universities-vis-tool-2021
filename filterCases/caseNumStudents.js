...
let allData = []; // All the data

const caseNumStudents = (data, enter, selectedCountry) => {

    arrayOfData.length = 0;
    // Limit the amount of data to display
    const limit = 30;
    ...
    arrayOfData.sort((a, b) => {
        const numStudentsA = parseInt(a['number students'].replace(',', ''));
        const numStudentsB = parseInt(b['number students'].replace(',', ''));
        return numStudentsB - numStudentsA;
    });
    ...
    const groupsOfMostAttenders = (width / amount_of_groups) - 25;
    ...
    try {
        customMax = d3.max(arrayOfData, data => parseInt(data['number students'].replace(',', '')))
        maxStudents = Math.max(0, customMax)
        minStudents = parseInt(arrayOfData[arrayOfData.length - 1]['number students'].replace(',', ''))
    } catch (e) {
        if (arrayOfData.length < 30) {
            maxStudents = parseInt(arrayOfData[0]['number students'].replace(',', ''))
            minStudents = parseInt(arrayOfData[arrayOfData.length - 1]['number students'].replace(',', ''))
        }
    }
    ...
    // Crear grupos para cada ubicación
    ...
    // Agregar etiquetas de texto fuera del contenedor de las barras
    ...
    const addLegend = () => {
        ...
        const values = [maxStudents]; // Valores inicial y final
    ...
    locationGroups.selectAll("rect") // Create the bars of the chart
        ...
        .attr("width", data =>
            xScale(parseInt(data['number students'].replace(',', '')))
        )
        ...
        .on("mouseover", function (event, d) {
            ...
            for (let i = 0; i < uniTexts._groups[0].length; i++) {
                if (d.title === uniTexts._groups[0][i].innerHTML) {
                    tooltip.html(
                        "Number of students: " + d['number students'] + "<br>"
                    );
                }
                ...
                if (uniTexts._groups[0][i].innerHTML === d.title) {
                    ...
                    d3.select(uniTexts._groups[0][i])
                        .transition()
                        .duration(50) // Duración de la transición en milisegundos
        ...
        .on("click", function (event, d) {
            ...
            if (counterLabelClicks % 2 === 0) {
                ...
                tooltip.style("border", "1px solid white")
                    .html("Number of students: " + d['number students'] + "<br>")
            }
...
export default caseNumStudents;