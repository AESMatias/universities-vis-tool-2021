...
const caseTeaching = (data, enter, selectedCountry) => {
    ...
    arrayOfData.sort((a, b) => {
        const numRankingA = parseInt(a['teaching score']);
        const numRankingB = parseInt(b['teaching score']);
        return numRankingB - numRankingA;
    });
    ...
    try {
        maxRank = d3.max(arrayOfData, data => parseInt(data['teaching score']))
        minRank = parseInt(arrayOfData[arrayOfData.length - 1]['teaching score'])
    } catch (e) {
        if (arrayOfData.length < 30) {
            maxRank = parseInt(arrayOfData[0]['teaching score'])
            minRank = parseInt(arrayOfData[arrayOfData.length - 1]['teaching score'])
        }
    }
    ...
    locationGroups.selectAll("rect") // Create the bars of the chart
        ...
        .attr("x", 420)
        .attr("width", data =>
            xScale(parseInt(data['teaching score']))
        )
        ...
        .on("mouseover", function (event, d) {
            ...
            for (let i = 0; i < uniTexts._groups[0].length; i++) {
                if (d.title === uniTexts._groups[0][i].innerHTML) {
                    tooltip.html(
                        "Teaching score: " + d['teaching score'] + "<br>"
                    );
                }
        ...
        .on("click", function (event, d) {
            ...
            if (counterLabelClicks % 2 === 0) {
                ...
                tooltip.style("border", "1px solid white")
                    .html("Teaching score: " + d['teaching score'] + "<br>")
            }
