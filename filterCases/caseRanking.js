...
const caseRanking = (data, enter, selectedCountry) => {
    ...
    arrayOfData.sort((a, b) => {
        const numRankingA = parseInt(a['ranking']);
        const numRankingB = parseInt(b['ranking']);
        return numRankingA - numRankingB;
    });
    ...
    try {
        maxRank = d3.max(arrayOfData, data => parseInt(data['ranking']))
        minRank = d3.min(arrayOfData, data => parseInt(data['ranking']))
    } catch (e) {
        console.log('error', e)
    ...
    const uniTexts = svg.selectAll(".uni-text")
        ...
        .attr("x", 0)
        // EL siguiente atributo usa funciones sin aparente sentido, pero no causan problemas.
