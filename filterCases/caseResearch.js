...
const caseResearch = (data, enter, selectedCountry) => {
    ...
    arrayOfData.sort((a, b) => {
        const numResearchA = parseInt(a['research score']);
        const numResearchB = parseInt(b['research score']);
        return numResearchB - numResearchA;
    });
    ...
    try {
        maxRank = d3.max(arrayOfData, data => parseInt(data['research score']))
        minRank = parseInt(arrayOfData[arrayOfData.length - 1]['research score'])
    } catch (e) {
        if (arrayOfData.length < 30) {
            maxRank = parseInt(arrayOfData[0]['research score'])
            minRank = parseInt(arrayOfData[arrayOfData.length - 1]['research score'])
        }
    }
    ...
    locationGroups.selectAll("rect") // Create the bars of the chart
        ...
        .attr("x", 420)
        .attr("width", data =>
            xScale(parseInt(data['research score']))
        )
        ...
        .on("click", function (event, d) {
            ...
            if (counterLabelClicks % 2 === 0) {
                ...
                tooltip.style("border", "1px solid white")
                    .html("Research score: " + d['research score'] + "<br>")
            }
