...
const caseIndustry = (data, enter, selectedCountry) => {
    ...
    arrayOfData.sort((a, b) => {
        const numIndustryA = parseInt(a['industry income score']);
        const numIndustryB = parseInt(b['industry income score']);
        return numIndustryB - numIndustryA;
    });
    ...
    try {
        maxRank = d3.max(arrayOfData, data => parseInt(data['industry income score']))
        minRank = parseInt(arrayOfData[arrayOfData.length - 1]['industry income score'])
    } catch (e) {
        if (arrayOfData.length < 30) {
            maxRank = parseInt(arrayOfData[0]['industry income score'])
            minRank = parseInt(arrayOfData[arrayOfData.length - 1]['industry income score'])
    ...
    const addLegend = () => {
        ...
        const values = [maxRank]; // Valores inicial y final
    ...
    locationGroups.selectAll("rect") // Create the bars of the chart
        ...
        .attr("x", 420)
        .attr("width", data =>
            xScale(parseInt(data['industry income score']))
        )
        ...
        .on("mouseover", function (event, d) {
            ...
            for (let i = 0; i < uniTexts._groups[0].length; i++) {
                ...
                if (uniTexts._groups[0][i].innerHTML === d.title) {
                    ...
                    //translation
                    // uniTexts._groups[0][i].setAttribute("transform", "translate(40, 0)")
        ...
        .on("click", function (event, d) {
            ...
            if (counterLabelClicks % 2 === 0) {
                ...
                tooltip.style("border", "1px solid white")
                    .html("Industry income score: " + d['industry income score'] + "<br>")
            }
